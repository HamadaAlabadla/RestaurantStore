...
namespace RestauranteStore.Infrastructure.Services.AdminService
{
	public class AdminUserService : IAdminUserService
	{
		...
		public async Task<ErrorModel> CreateAdminWithUser(AdminDto adminDto)
		{
			...
			var userResult = await userService.CreateUser(user, "Admin");
			...
			var logoPath = await fileService.UploadFile(adminDto.Logo!, user.UserName!);
			admin.Logo = logoPath;
		...
		public async Task<object?> GetAllAdminsWithUsers(int pageLength, int skiped, StringValues searchData, StringValues sortColumn, StringValues sortDir)
		{
			...
			var adminsViewModel = mapper.Map<IEnumerable<UserViewModel>>(data);
			foreach (var item in adminsViewModel)
			{
				item.Role = await userService.GetRoleByUser(item.userId ?? "");
			}
		...
		public async Task<ErrorModel> UpdateAdminWithUser(AdminDto adminDto)
		{
			...
			{
				var logoPath = await fileService.UploadFile(adminDto.Logo!, userDb.UserName!);
				admin.Logo = logoPath;
			}
			else
				admin.Logo = pathLogo;
